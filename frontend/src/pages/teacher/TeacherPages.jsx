...
export const teacherNav = [
  ...
  { path: "/teacher/chat",          label: "SmartBot",       },
  { path: "/teacher/alerts",        label: "Alerts",         },
];

const card = { background: "#fff", border: "1px solid #e2e8f0", borderRadius: 12, padding: 22 };
const input = { width: "100%", padding: "10px 13px", background: "#f8f9fb", border: "1px solid #e2e8f0", borderRadius: 8, color: "#1e293b", fontSize: 14, outline: "none", boxSizing: "border-box", fontFamily: "inherit" };
const btn = (color = "#4f46e5") => ({ padding: "10px 20px", background: color, border: "none", borderRadius: 8, color: "#fff", fontSize: 14, fontWeight: 600, cursor: "pointer" });
              ...
              <div key={s._id} style={{ ...card, padding: 0, overflow: "hidden" }}>
                ...
                <div style={{ padding: "20px 24px", display: "flex", alignItems: "center", justifyContent: "space-between", borderLeft: `4px solid ${color}` }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
                    ...
                    <div>
                      <div style={{ color: "#1e293b", fontSize: 16, fontWeight: 700 }}>{s.name}</div>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                    <div style={{ textAlign: "right" }}>
                      <div style={{ color: "#1e293b", fontSize: 18, fontWeight: 700 }}>{s.students?.length || 0}</div>
                    ...
                    {!subData?.loading && subData?.students?.length > 0 && (
                      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(220px, 1fr))", gap: 10 }}>
                        {subData.students.map((st, j) => (
                          <div key={j} style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 14px", borderRadius: 10, background: "rgba(15,23,42,0.1)", border: "1px solid rgba(255,255,255,0.06)" }}>
                            ...
                            <div>
                              <div style={{ color: "#1e293b", fontSize: 13, fontWeight: 600 }}>{st.name}</div>
          ...
          <div style={card}>
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              ...
              <tbody>
                ...
                {filtered.map((s, i) => (
                  <tr key={i}>
                    <td style={{ padding: "12px 16px", borderBottom: "1px solid #f8f9fb" }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                        ...
                        <span style={{ color: "#1e293b", fontSize: 14, fontWeight: 600 }}>{s.name}</span>
                      </div>
                    </td>
                    <td style={{ padding: "12px 16px", color: "#64748b", fontSize: 13, borderBottom: "1px solid #f8f9fb" }}>{s.email}</td>
                    <td style={{ padding: "12px 16px", borderBottom: "1px solid #f8f9fb" }}>
              ...
              <div style={modalBg} onClick={e => e.stopPropagation()}>
                ...
                <div style={{ padding: "20px 24px", borderBottom: "1px solid #f1f5f9", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
                    ...
                    <div>
                      <div style={{ color: "#64748b", fontSize: 17, fontWeight: 700 }}>{detail?.name || "Loading..."}</div>
                          ...
                          <table style={{ width: "100%", borderCollapse: "collapse" }}>
                            <thead>
                              <tr>{["Subject","Exam","Obtained","Max","%"].map(h => (
                                <th key={h} style={{ color: "#64748b", fontSize: 11, textTransform: "uppercase", padding: "0 12px 12px", textAlign: "left", borderBottom: "1px solid #f1f5f9" }}>{h}</th>
                              ))}</tr>
                            </thead>
                            <tbody>
                              {detail.marks?.map((m, i) => {
                                ...
                                return (
                                  <tr key={i}>
                                    <td style={{ padding: "10px 12px", color: "#1e293b", fontSize: 13, borderBottom: "1px solid #f8f9fb" }}>{m.subject}</td>
                                    <td style={{ padding: "10px 12px", borderBottom: "1px solid #f8f9fb" }}><span style={{ padding: "2px 8px", borderRadius: 6, background: "rgba(99,102,241,0.15)", color: "#818cf8", fontSize: 11 }}>{m.examType}</span></td>
                                    <td style={{ padding: "10px 12px", color: "#0f172a", fontWeight: 700, borderBottom: "1px solid #f8f9fb" }}>{m.marksObtained}</td>
                                    <td style={{ padding: "10px 12px", color: "#64748b", borderBottom: "1px solid #f8f9fb" }}>{m.maxMarks}</td>
                                    <td style={{ padding: "10px 12px", borderBottom: "1px solid #f8f9fb" }}>
                      ...
                      {detailTab === "attendance" && (
                        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                          ...
                          {detail.attendanceStats?.map((a, i) => (
                            <div key={i} style={{ display: "flex", alignItems: "center", gap: 14, padding: "12px 16px", borderRadius: 10, background: "#f8f9fb" }}>
                              <div style={{ flex: 1 }}>
                                <div style={{ color: "#1e293b", fontSize: 14, fontWeight: 600 }}>{a.subject}</div>
                      ...
                      {detailTab === "submissions" && (
                        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                          ...
                          {detail.submissions?.map((s, i) => (
                            <div key={i} style={{ padding: "12px 16px", borderRadius: 10, background: "#f8f9fb" }}>
                              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                                <div>
                                  <div style={{ color: "#1e293b", fontSize: 14, fontWeight: 600 }}>{s.assignment}</div>
          ...
          {assignments.map(a => (
            <div key={a.id} style={{ ...card, padding: 0, overflow: "hidden" }}>
              <div style={{ padding: "18px 22px", display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12 }}>
                <div>
                  <div style={{ color: "#1e293b", fontWeight: 600, fontSize: 15 }}>{a.title}</div>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  ...
                  <button onClick={() => toggleResub(a.id, a.allowResubmission)}
                    ...
                    style={{ padding: "7px 12px", borderRadius: 8, fontSize: 12, fontWeight: 600, cursor: "pointer", background: a.allowResubmission ? "rgba(99,102,241,0.15)" : "rgba(30,41,59,0.2)", border: `1px solid ${a.allowResubmission ? "rgba(99,102,241,0.4)" : "#e2e8f0"}`, color: a.allowResubmission ? "#818cf8" : "#64748b", whiteSpace: "nowrap" }}>
                  ...
                  {!subsLoading && submissions.map(s => (
                    <div key={s._id} style={{ marginBottom: 16, padding: 16, borderRadius: 10, background: "rgba(15,23,42,0.6)", border: "1px solid rgba(255,255,255,0.06)" }}>
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 12, marginBottom: 10 }}>
                        <div>
                          <div style={{ color: "#1e293b", fontWeight: 600, fontSize: 14 }}>{s.student} <span style={{ color: "#64748b", fontSize: 12 }}>({s.rollNumber})</span></div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(240px, 1fr))", gap: 16 }}>
          ...
          {data.map((r) => (
            <div key={r._id} style={card}>
              <div style={{ color: "#1e293b", fontSize: 15, fontWeight: 600 }}>{r.name}</div>
                  ...
                  {["Info","Warning","Emergency"].map(l => {
                    ...
                    return (
                      <button key={l} onClick={() => setForm(f => ({...f, level: l}))}
                        style={{ padding: "8px 4px", borderRadius: 8, fontSize: 12, fontWeight: 600, cursor: "pointer", border: `2px solid ${form.level === l ? cfg.color : "#e2e8f0"}`, background: form.level === l ? cfg.bg : "#f1f5f9", color: form.level === l ? cfg.color : "#64748b" }}>
                        {cfg.icon} {l}
                      </button>
                    );
                  })}
                  ...
                  <div style={{ flex: 1 }}>
                    <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6, flexWrap: "wrap", gap: 6 }}>
                      <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                        ...
                        <span style={{ padding: "1px 7px", borderRadius: 20, background: "#f1f5f9", color: "#64748b", fontSize: 11 }}>→ {a.targetRole}</span>
                      </div>
